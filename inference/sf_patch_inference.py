#!/usr/bin/env python3
"""
SpectralFormer patch-wise INFERENCE for the EBS-only polymer dataset.

This is the inference-only counterpart of ../sf_patch.py. It loads a trained
SpectralFormer (ViT, mode="CAF") checkpoint (produced by sf_patch.py) and
runs patch-wise prediction on the EBS-only test cubes, followed by
semantic/SAM-instance majority voting and evaluation against ground truth.
Model architecture, data loading/prepping, prediction, evaluation, and
visualisation code are kept unchanged; the training loop, grid search, and
validation-split machinery have been removed.
"""
...
OUTPUT_ROOT = DATASET_ROOT / "trained_models" / "SF_patch"
...
# ---------------------------------------------------------------------------
# SpectralFormer (patch) inference defaults. Architecture params must match
# the checkpoint produced by sf_patch.py (grid search always used
# patch_size=9, band_patch=7, vit_depth=5).
# ---------------------------------------------------------------------------
...
# TRIMMED from sf_patch.py: training cubes/GT, the 6-patch validation split,
...
def load_prepared_data(dataset_root: Path) -> Dict[str, Any]:
    """
    Load and prepare EBS-only test cubes using the same split as sf_patch.py.
    ...
    # Same test split as KNN.py / sf_patch.py (index 0 of each class == test cube)
    test_cubes = [
...
# (Renamed/simplified from sf_patch.py's run_best_model_outputs: no
...
def run_inference(
    ...
    copied_model_path = output_root / "sf_patch_model.pth"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run SpectralFormer patch-wise INFERENCE for EBS-only data."
    )
    parser.add_argument("--dataset-root", type=Path, default=DATASET_ROOT)
    parser.add_argument("--output-root",  type=Path, default=None)
    parser.add_argument(
        "--model-path", type=Path, required=True,
        help="Path to a trained SpectralFormer checkpoint (.pth) produced by sf_patch.py.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    output_root = (
        ...
        else dataset_root / "trained_models" / "SF_patch" / "inference"
    )
    output_root.mkdir(parents=True, exist_ok=True)
    ...
    print("\n" + "=" * 80)
    print("SpectralFormer (patch) inference complete")
    print(json.dumps(
        json_ready({
            "patch_size": args.patch_size,
            "band_patch": args.band_patch,
