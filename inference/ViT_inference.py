#!/usr/bin/env python3
"""
ViT patch-wise INFERENCE for the EBS-only polymer dataset.

This is the inference-only counterpart of ../ViT.py. It loads a trained ViT
checkpoint (produced by ViT.py) and runs patch-wise prediction on the
...
# ---------------------------------------------------------------------------
# ViT inference defaults. Architecture params must match the checkpoint that
# was produced by ViT.py (grid search always used patch_size=9, band_patch=1,
# vit_depth=5).
# ---------------------------------------------------------------------------

DEFAULT_PATCH_SIZE = 9
DEFAULT_BATCH_SIZE = 512
DEFAULT_BAND_PATCH = 1   # For ViT, band_patch is always 1 (no GSE)
DEFAULT_VIT_DEPTH = 5
...
# ---------------------------------------------------------------------------
# For ViT: Single band_patch, i.e. no GSE (identity transform). Still needed
# by predict_maps below (unchanged from ViT.py).
# ---------------------------------------------------------------------------
...
# TRIMMED from ViT.py: training cubes/GT, the 6-patch validation split, and
...
def load_prepared_data(dataset_root: Path) -> Dict[str, Any]:
    """
    Load and prepare EBS-only test cubes using the same split as ViT.py.
    ...
    # Same test split as KNN.py / ViT.py (index 0 of each class == test cube)
    test_cubes = [
...
# ---------------------------------------------------------------------------
# Inference run: apply the loaded ViT model to the test cubes, run majority
# voting, evaluate, and save outputs.
# (Renamed/simplified from ViT.py's run_best_model_outputs: no grid-search
...
def run_inference(
    ...
    """Apply the ViT model to test cubes, run majority voting, evaluate, and save outputs."""
    ...
    copied_model_path = output_root / "vit_model.pth"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run ViT patch-wise INFERENCE for EBS-only data."
    )
    parser.add_argument("--dataset-root", type=Path, default=DATASET_ROOT)
    parser.add_argument("--output-root",  type=Path, default=None)
    parser.add_argument(
        "--model-path", type=Path, required=True,
        help="Path to a trained ViT checkpoint (.pth) produced by ViT.py.",
    )
    parser.add_argument(
        "--patch-size", type=int, default=DEFAULT_PATCH_SIZE,
        help="Must match the patch_size used to train the checkpoint (default: 9).",
    )
    parser.add_argument(
        "--band-patch", type=int, default=DEFAULT_BAND_PATCH,
        help="Must match the band_patch used to train the checkpoint (default: 1, ViT has no GSE).",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    output_root = (
        ...
        else dataset_root / "trained_models" / "ViT_patch" / "inference"
    )
    output_root.mkdir(parents=True, exist_ok=True)
