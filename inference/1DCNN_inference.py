#!/usr/bin/env python3
"""
1DCNN pixel-wise INFERENCE for the EBS-only polymer dataset.

This is the inference-only counterpart of ../1DCNN.py. It loads a trained
1DCNN checkpoint (produced by 1DCNN.py) and runs pixel-wise prediction on the
EBS-only test cubes, followed by semantic/SAM-instance majority voting and
evaluation against ground truth. Model architecture, data loading/prepping,
prediction, evaluation, and visualisation code are kept unchanged; the
training loop, grid search, and validation-split machinery have been removed.
"""
...
# ---------------------------------------------------------------------------
# 1DCNN inference defaults. Architecture params must match the checkpoint
# that was produced by 1DCNN.py (grid search always used band_patch=1).
# ---------------------------------------------------------------------------

DEFAULT_BATCH_SIZE = 512
DEFAULT_BAND_PATCH = 1  # 1DCNN training always used band_patch=1

# 1DCNN parameters from EBS_only/1DCNN.ipynb, must match training
CNN_KERNEL_SIZE = 3
CNN_CHANNELS = 32
CNN_DROPOUT = 0.1

# ---------------------------------------------------------------------------
# 1DCNN Model Definition
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Model building: build the 1DCNN architecture that matches the checkpoint
# (unchanged from 1DCNN.py, minus vit_depth which never applied here).
# ---------------------------------------------------------------------------
def build_model(
    band_patch: int,
    num_classes: int,
    ...
    # "band_patch" produces input shape (bands, band_patch)
...
# TRIMMED from 1DCNN.py: training cubes/GT, the 6-patch validation split, and
# the class-weights tensor are training-only and have been removed. Only the
# test cubes (and their GT/semantic/instance masks) needed for inference are
# kept, using the same class split as the training script.
# ---------------------------------------------------------------------------
def load_prepared_data(dataset_root: Path) -> Dict[str, Any]:
    """
    Load and prepare EBS-only test cubes using the same split as 1DCNN.py.
    ...
    # Same test split as KNN.py / 1DCNN.py (index 0 of each class == test cube)
    test_cubes = [
...
# ---------------------------------------------------------------------------
# Inference run: apply the loaded 1DCNN model to the test cubes, run majority
# voting, evaluate, and save outputs.
# (Renamed/simplified from 1DCNN.py's run_best_model_outputs: no grid-search
# "best experiment" selection anymore -- the model is loaded directly from
# --model-path.)
# ---------------------------------------------------------------------------
def run_inference(
    model: nn.Module,
    ...
    """Apply the 1DCNN model to test cubes, run majority voting, evaluate, and save outputs."""
    ...
    copied_model_path = output_root / "1DCNN_model.pth"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run 1DCNN pixel-wise INFERENCE for EBS-only data."
    )
    parser.add_argument("--dataset-root", type=Path, default=DATASET_ROOT)
    parser.add_argument("--output-root",  type=Path, default=None)
    parser.add_argument(
        "--model-path", type=Path, required=True,
        help="Path to a trained 1DCNN checkpoint (.pth) produced by 1DCNN.py.",
    )
    parser.add_argument(
        "--band-patch", type=int, default=DEFAULT_BAND_PATCH,
        help="Must match the band_patch used to train the checkpoint (default: 1).",
    )
...
def main() -> None:
    ...
    output_root = (
        ...
        else dataset_root / "trained_models" / "1DCNN" / "inference"
    )
    output_root.mkdir(parents=True, exist_ok=True)
    ...
    model = build_model(band_patch=args.band_patch, num_classes=len(CLASS_LABELS))
    ...
    print("\n" + "=" * 80)
    print("1DCNN inference complete")
    print(json.dumps(json_ready({"band_patch": args.band_patch}), indent=2))
    print(
