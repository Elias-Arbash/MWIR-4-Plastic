#!/usr/bin/env python3
"""
Random Forest experiments for the EBS-only polymer dataset.

This script mirrors KNN.py:
load EBS12/EBS22, SNV-normalize FENIX and FX50, keep fingerprint bands,
concatenate sensors, train/test split by sample, train Random Forest models,
and evaluate pixel-wise plus semantic/SAM-instance majority-voted prediction maps.
"""
...
OUTPUT_ROOT = DATASET_ROOT / "trained_models" / "RF"
...
def build_rf_pipeline(
    ...
    """Create a StandardScaler + Random Forest model for one experiment."""
    return Pipeline(
        [
...
def make_experiments(max_experiments: int) -> List[Dict[str, Any]]:
    """Sample up to max_experiments combinations from the RF parameter grid."""
    ...
    for exp_number, combo_index in enumerate(selected_indices, start=1):
        n_estimators, max_depth, max_features, min_samples_leaf, class_weight = all_combinations[combo_index]
        experiments.append(
            {
...
def run_experiment(
    ...
    """Train, evaluate, and save one Random Forest experiment."""
    exp_number = exp["exp_number"]
    exp_dir = output_root / f"RF_exp_{exp_number}"
    ...
    model_path = exp_dir / "rf_model.pkl"
...
def run_best_model_outputs(best: Dict[str, Any], prepared: Dict[str, Any], output_root: Path) -> Dict[str, Any]:
    """Apply best RF model to maps, run majority voting, evaluate, and save outputs."""
    ...
    best_model_path = best_dir / "rf_best_model.pkl"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Random Forest grid search for EBS-only data.")
...
def main() -> None:
    ...
    output_root = (
        ...
        else dataset_root / "trained_models" / "RF"
    )
    output_root.mkdir(parents=True, exist_ok=True)

    log("=" * 80)
    log("EBS-only Random Forest experiment run started")
    ...
    log(f"[grid] Running {len(experiments)} RF experiments")

    all_results = []
    for exp in experiments:
        log(
            f"[experiment {exp['exp_number']}/{len(experiments)}] "
            f"n_estimators={exp['n_estimators']}, max_depth={exp['max_depth']}, "
            f"max_features={exp['max_features']}, min_samples_leaf={exp['min_samples_leaf']}, "
            f"class_weight={exp['class_weight']}"
        )
    ...
    log("EBS-only Random Forest experiment run finished")
    log("=" * 80)
