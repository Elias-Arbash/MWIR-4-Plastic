#!/usr/bin/env python3
"""
KNN experiments for the EBS-only polymer dataset.

This script follows the workflow from KNN.ipynb:
load EBS12/EBS22, SNV-normalize FENIX and FX50, keep fingerprint bands,
concatenate sensors, train/test split by sample, train KNN models, and evaluate
...
OUTPUT_ROOT = DATASET_ROOT / "trained_models" / "KNN"
...
def build_knn_pipeline(
    ...
    """Create a StandardScaler + KNN model for one experiment."""
    return Pipeline(
        [
...
def make_experiments(max_experiments: int) -> List[Dict[str, Any]]:
    """Sample up to max_experiments combinations from the KNN parameter grid."""
    ...
    for exp_number, combo_index in enumerate(selected_indices, start=1):
        n_neighbors, weights, algorithm, metric = all_combinations[combo_index]
        experiments.append(
            {
...
def run_experiment(
    ...
    """Train, evaluate, and save one KNN experiment."""
    exp_number = exp["exp_number"]
    exp_dir = output_root / f"KNN_exp_{exp_number}"
    ...
    model_path = exp_dir / "knn_model.pkl"
...
def run_best_model_outputs(best: Dict[str, Any], prepared: Dict[str, Any], output_root: Path) -> Dict[str, Any]:
    """Apply best KNN model to maps, run majority voting, evaluate, and save outputs."""
    ...
    best_model_path = best_dir / "knn_best_model.pkl"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run KNN grid search for EBS-only data.")
...
def main() -> None:
    ...
    output_root = (
        ...
        else dataset_root / "trained_models" / "KNN"
    )
    ...
    print(f"Running {len(experiments)} KNN experiments")

    all_results = []
    for exp in experiments:
        print(
            f"[{exp['exp_number']}/{len(experiments)}] "
            f"k={exp['n_neighbors']}, weights={exp['weights']}, "
            f"algorithm={exp['algorithm']}, metric={exp['metric']}"
        )
