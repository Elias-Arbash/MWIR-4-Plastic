#!/usr/bin/env python3
"""
SpectralFormer patch-wise experiments for the EBS-only polymer dataset.
...
set, train SpectralFormer (ViT) models with early stopping, then evaluate
...
class HSIDataset(Dataset):
    """
    Patch-based HSI dataset compatible with SpectralFormer.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run SpectralFormer patch-wise grid search for EBS-only data."
    )
...
def main() -> None:
    ...
    output_root = (
        ...
        else dataset_root / "trained_models" / "SF_patch"
    )
