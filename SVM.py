#!/usr/bin/env python3
"""
SVM experiments for the EBS-only polymer dataset.

This script mirrors KNN.py:
load EBS12/EBS22, SNV-normalize FENIX and FX50, keep fingerprint bands,
concatenate sensors, train/test split by sample, train SVM models, and evaluate
pixel-wise plus semantic/SAM-instance majority-voted prediction maps.

The classifier uses SGDClassifier(loss="hinge"), a scalable linear SVM trainer
that is practical for the large pixel-wise training set used here.
"""
...
OUTPUT_ROOT = DATASET_ROOT / "trained_models" / "SVM"
...
ALPHA_OPTIONS = [1e-5, 3e-5, 1e-4, 3e-4, 1e-3]
PENALTY_OPTIONS = ["l2", "l1", "elasticnet"]
CLASS_WEIGHT_OPTIONS = [None, "balanced"]
AVERAGE_OPTIONS = [False, True]
L1_RATIO_OPTIONS = [0.15, 0.35]
DEFAULT_EXPERIMENTS = 25
RANDOM_STATE = 42


def log(message: str) -> None:
    """Print progress immediately so SLURM .out files show live status."""
...
def predict_maps(
    ...
    for index, (cube, valid_mask) in enumerate(zip(cubes, valid_masks), start=1):
        log(f"[best model] Predicting map {index}/{len(cubes)}")
        cube = np.asarray(cube)
        spectra = cube[valid_mask]
...
def build_svm_pipeline(
    ...
    """Create a StandardScaler + linear SVM model for one experiment."""
    return Pipeline(
        [
...
def make_experiments(max_experiments: int) -> List[Dict[str, Any]]:
    """Sample up to max_experiments combinations from the SVM parameter grid."""
    ...
    for exp_number, combo_index in enumerate(selected_indices, start=1):
        alpha, penalty, class_weight, average, l1_ratio = all_combinations[combo_index]
        experiments.append(
            {
                "exp_number": exp_number,
                "algorithm": "linear_sgd_svm",
                "loss": "hinge",
                "alpha": float(alpha),
                "penalty": penalty,
                "class_weight": class_weight,
...
def load_prepared_data(dataset_root: Path) -> Dict[str, Any]:
    ...
    log(f"[data] Loaded records: {len(records)}")
    log("[data] Extracting FENIX/FX50 cubes and masks")
    ...
    for class_name, class_slice in class_slices.items():
        log(f"[data] Normalizing and selecting ROI bands for {class_name}")
        ...
        log(f"[data] {class_name}: {len(by_class[class_name]['cubes'])} cubes ready")

    log("[data] Building notebook train/test split")
    train_cubes = (
    ...
    log("[data] Extracting train/test pixel vectors")
    ...
    log(f"[data] train_X: {train_x.shape}, train_y: {train_y.shape}")
    log(f"[data] test_X: {test_x.shape}, test_y: {test_y.shape}")
    log(f"[data] Train labels: {dict(zip(*np.unique(train_y, return_counts=True)))}")
    log(f"[data] Test labels: {dict(zip(*np.unique(test_y, return_counts=True)))}")
...
def save_prediction_maps(
    ...
    for sample_name, pixel_map, semantic_map, instance_map in zip(
        ...
        log(f"[best model] Saved prediction maps for {sample_name}")
...
def run_experiment(
    ...
    """Train, evaluate, and save one SVM experiment."""
    exp_number = exp["exp_number"]
    exp_dir = output_root / f"SVM_exp_{exp_number}"
    exp_dir.mkdir(parents=True, exist_ok=True)

    log(f"[experiment {exp_number}] Building model")
    ...
    log(f"[experiment {exp_number}] Training started")
    model.fit(train_x, train_y)
    log(f"[experiment {exp_number}] Training finished")

    log(f"[experiment {exp_number}] Pixel-wise prediction started")
    y_pred = model.predict(test_x)
    log(f"[experiment {exp_number}] Pixel-wise prediction finished")

    pixel_metrics = evaluate_maps([test_y], [y_pred], labels=CLASS_LABELS, class_labels=CLASS_LABELS, ignore_labels=())
    model_path = exp_dir / "svm_model.pkl"
    ...
    log(f"[experiment {exp_number}] Saved model and results to {exp_dir}")
    return result


def run_best_model_outputs(best: Dict[str, Any], prepared: Dict[str, Any], output_root: Path) -> Dict[str, Any]:
    """Apply best SVM model to maps, run majority voting, evaluate, and save outputs."""
    best_dir = output_root / "best_model"
    best_dir.mkdir(parents=True, exist_ok=True)
    log("[best model] Loading selected model")
    model = joblib.load(best["model_path"])

    log("[best model] Creating pixel-wise prediction maps")
    pixel_maps = predict_maps(model, prepared["test_cubes"], prepared["test_valid_masks"])

    log("[best model] Applying semantic-object majority voting")
    semantic_maps = [
    ...
    log("[best model] Applying SAM-instance majority voting")
    instance_maps = [
        majority_vote_sam_instances(
            pred_map,
            instances,
    ...
    log("[best model] Evaluating pixel-wise and majority-voted maps")
    evaluations = {
    ...
    best_model_path = best_dir / "svm_best_model.pkl"
    ...
    log("[best model] Saving pixel-wise, semantic-majority, and instance-majority maps")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SVM grid search for EBS-only data.")
...
def main() -> None:
    ...
    output_root = (
        ...
        else dataset_root / "trained_models" / "SVM"
    )
    output_root.mkdir(parents=True, exist_ok=True)

    log("=" * 80)
    log("EBS-only SVM experiment run started")
    ...
    log(f"[grid] Running {len(experiments)} SVM experiments")

    all_results = []
    for exp in experiments:
        log(
            f"[experiment {exp['exp_number']}/{len(experiments)}] "
            f"alpha={exp['alpha']}, penalty={exp['penalty']}, "
            f"class_weight={exp['class_weight']}, average={exp['average']}, "
            f"l1_ratio={exp['l1_ratio']}"
        )
        ...
        log(f"[experiment {exp['exp_number']}] average class kappa: {result['ranking_score']:.4f}")
    ...
    log(f"[grid] Saved sorted experiment results to {output_root / 'all_experiment_results.json'}")

    best_summary = run_best_model_outputs(best, prepared, output_root)
    log("=" * 80)
    log("Best SVM experiment")
    log(json.dumps(json_ready(best_summary["selected_experiment"]), indent=2))
    log(f"Average class kappa: {best_summary['ranking_score_average_class_kappa']:.4f}")
    log(f"Saved outputs to: {output_root / 'best_model'}")
    log("EBS-only SVM experiment run finished")
    log("=" * 80)
