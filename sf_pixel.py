#!/usr/bin/env python3
"""
SpectralFormer pixel-wise experiments for the EBS-only polymer dataset.

This script trains SpectralFormer (ViT) pixel-wise: patch_size=1, i.e. each "patch"
is a single pixel spectrum. All logic mirrors SF_patch.py except for the patch size.
Results and models are saved to <output_root>/SF_pixel/. 

Pixel-wise evaluation, and additional semantic/SAM-instance majority voting are performed.
Grid search is performed over band_patch and vit_depth.  The best model is
...
# ---------------------------------------------------------------------------
# SpectralFormer default hyperparameters (from SF_patch.ipynb)
# Only difference in pixel version: patch size always = 1, path names, folder names.
# ---------------------------------------------------------------------------
...
class HSIDataset(Dataset):
    """
    Pixel-wise HSI dataset compatible with SpectralFormer.
...
def make_experiments(max_experiments: int) -> List[Dict[str, Any]]:
    """Sample up to *max_experiments* combinations from the SF parameter grid."""
...
def run_experiment(
    ...
    """Train one SpectralFormer configuration and evaluate pixel-wise on test cubes."""
    exp_number = exp["exp_number"]
    patch_size = 1  # always pixel-wise
    band_patch = exp["band_patch"]
    vit_depth = exp["vit_depth"]

    exp_dir = output_root / f"SF_exp_{exp_number}"
    exp_dir.mkdir(parents=True, exist_ok=True)
    model_path = exp_dir / "sf_model.pth"
...
def run_best_model_outputs(
    ...
    """Apply best SF model to maps, run majority voting, evaluate, and save outputs."""
    ...
    best_model_path = best_dir / "sf_best_model.pth"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run SpectralFormer pixel-wise grid search for EBS-only data."
    )
...
def main() -> None:
    ...
    output_root = (
        ...
        else dataset_root / "trained_models" / "SF_pixel"
    )
    ...
    print(f"\nRunning {len(experiments)} SpectralFormer experiment(s)")
    ...
    print("\n" + "=" * 80)
    print("Best SpectralFormer experiment")
